...
abstract class UHFReaderSLRBase extends UHFReaderBase implements IUHFReader {
    ...
    @Override
    public UHFReaderResult connect() {
        return null;
    }
    ...
    public UHFReaderResult<Boolean> setPower(int power) {
        ...
        if (dataFrameInfo != null) {
            Log.e("ttt", "setPower: status = " + dataFrameInfo );
            int status = dataFrameInfo.status;
            if (status == 261) {
                DataFrameInfo dataFrameInfo2 = sendAndReceiveData(builderAnalysisSLR.makeSetPowerSendData(30));
    ...
    public UHFReaderResult<Boolean> kill(String password, SelectEntity selectEntity) {
        ...
        Log.e("TAG", "kill:+++++ " + DataConverter.bytesToHex(dataFrameInfo.data) );
