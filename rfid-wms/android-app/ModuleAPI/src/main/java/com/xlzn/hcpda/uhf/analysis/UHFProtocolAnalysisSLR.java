...
public class UHFProtocolAnalysisSLR extends UHFProtocolAnalysisBase implements IUHFProtocolAnalysis {
            ...
            if(listCmd!=null && listCmd.size()>0){
                synchronized (lock) {
                    for (int k = 0; k < listCmd.size(); k++) {
                        ...
                        LoggerUtils.d(TAG, "获取非连续盘点的标签数据 getOtherInfo =" + DataConverter.bytesToHex(dataFrameInfo.data));
    ...
    protected void addData(DataFrameInfo dataFrameInfo){
        LoggerUtils.d(TAG, "添加标签数据---command--- "+dataFrameInfo.command);
        LoggerUtils.d(TAG, "添加标签数据---status--- "+dataFrameInfo.status);
        if(dataFrameInfo.command==0x29){
            //标签数据
            LoggerUtils.d(TAG, "添加标签数据------2");
            if(dataFrameInfo.status==0) {
                LoggerUtils.d(TAG, "添加标签数据------3");
                byte[] taginfo = dataFrameInfo.data;
                int tagsTotal = taginfo[3] & 0xFF;//标签张数
                if (tagsTotal > 0) {
                    LoggerUtils.d(TAG, "添加标签数据------4");
            ...
            if(data!=null && data.length>=10){
                LoggerUtils.d(TAG, "添加标签数据  data.length ------ "+data.length);
                LoggerUtils.d(TAG, "添加标签数据  dataFrameInfo.status ------ "+dataFrameInfo.status);
              ...
              if(flag){
                  LoggerUtils.d(TAG, "开始盘点或者停止盘点的数据------flag");
                  //开始盘点或者停止盘点的数据
                  ...
                  if(dataFrameInfo.status==0) {
                      byte[] taginfo = dataFrameInfo.data;
                      int tagsTotal = taginfo[4] & 0xFF;//标签张数

                      //0082 E4 0060 E28011052000798365DA0973 10 3400300833B2DDD9014000000000 C41E
                      LoggerUtils.d(TAG, "添加标签数据 taginfo ------ " + DataConverter.bytesToHex(taginfo));
                      LoggerUtils.d(TAG, "添加标签数据 taginfo[4] ------ " + taginfo[4]);
                      queueTaginfo.offer(dataFrameInfo);
                      if (tagsTotal > 0) {
                          LoggerUtils.d(TAG, "增加标签数据.标签个数 = " + tagsTotal);
                          queueTaginfo.offer(dataFrameInfo);
                      } else {
                          LoggerUtils.d(TAG, "添加标签数据------ 没有解析到标签个数");
                      }
                  }
              }
            }
