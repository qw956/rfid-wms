...
public class UHFReaderSLR implements IUHFReader {
    ...
    @Override
    public UHFReaderResult<Boolean> connect() {
        ...
        LoggerUtils.d(TAG, "供电-------9");
        //****************
        UHFReaderResult<UHFVersionInfo> verInfo = null;
        int baudrate = 115200;
//        int baudrate = 921600;
        String uart = "/dev/ttysWK0";
        for (int k = 0; k < 2; k++) {
            if (isWK0) {
                uart = "/dev/ttysWK0";
            } else {
            ...
            Log.e(TAG, "connect:超高频连接串口  - -  " +uart );
            boolean result = UHFSerialPort.getInstance().open(uart, uhfProtocolAnalysisSLR, baudrate);
...
//                UHFSerialPort.getInstance().close();
                ...
                Log.e(TAG, "connect:超高频连接串口  - -  " +uart );
                boolean result2 = UHFSerialPort.getInstance().open(uart, uhfProtocolAnalysisSLR, baudrate);
                ...
                SystemClock.sleep(200);
                if (verInfo.getResultCode() == UHFReaderResult.ResultCode.CODE_SUCCESS) {
                    LoggerUtils.d(TAG, "模版版本----" + verInfo.getData().getHardwareVersion());
                    if (verInfo.getData().getHardwareVersion().startsWith("32") || verInfo.getData().getHardwareVersion().startsWith("31") || verInfo.getData().getHardwareVersion().startsWith("33")) {
                        builderAnalysisSLR = new BuilderAnalysisSLR_E710();
//                        builderAnalysisSLR = new BuilderAnalysisSLR();
                        LoggerUtils.d(TAG, "是E710啊----");
...
//                    hcPowerCtrl.uhfCtrl(0);
    ...
    public UHFReaderResult<Boolean> write(String password, int membank, int address, int wordCount, String data, SelectEntity selectEntity) {
        
    ...
    public UHFReaderResult<Boolean> writeEPCID(String password, int address, String selectData, String writeData) {
        return iuhfReader.writeEPCID(password,address,selectData,writeData);
    }
    ...
    public UHFReaderResult<Boolean> setGB(boolean enable) {
        return iuhfReader.setGB(enable);
    }
