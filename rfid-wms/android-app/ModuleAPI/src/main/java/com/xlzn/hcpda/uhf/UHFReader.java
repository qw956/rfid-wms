...
/**
 * UHF use process: Need to call first initialize the connect method,
 * used in exit pages or don't need to call the disConnect method,
 * set up before you start to move inventory correction setOnInventoryDataListener monitoring,
 * if the number of tag read more than 1000, You can set isMoreTag = true to read faster.
 * If the number of tags is small, do not set this parameter.
 */
public class UHFReader {
    ...
    public static boolean isLedKLV = false;
    ...
    public void setKLVLED(boolean flag) {
        isLedKLV = flag;
    }
     ...
     */
    public synchronized UHFReaderResult<Boolean> connect() {
        if (getConnectState() == ConnectState.CONNECTED) {
            return new UHFReaderResult(CODE_SUCCESS, "模块已经连接成功,请勿重复连接!", true);
        }

        UHFReaderResult result = UHFReaderSLR.getInstance().connect();
...
//TODO
    ...
    public UHFReaderResult<Boolean> setInventoryKLVLED(boolean flag) {
        LoggerUtils.d("TAG", "是否设置凯路威亮灯标签 = " + flag);

        if (getConnectState() != ConnectState.CONNECTED) {
            return new UHFReaderResult(CODE_READER_NOT_CONNECTED, UHFReaderResult.ResultMessage.READER_NOT_CONNECTED, false);
        }
        return reader.setInventoryKLVLED(flag);
    }
    ...
    public UHFReaderResult<Boolean> writeEPCID(String password, int address, String selectData, String writeData) {
        ...
        return reader.writeEPCID(password, address, selectData, writeData);
    }
     ...
     * @param membankEnum
     * @param actionEnum
     * @param selectEntity Specifies the label. If this parameter is null, no label is  Specifies the label. null indicates that no label is specified
     * @return UHFReaderResult
     */
    ...
    public UHFReaderResult<Boolean> setGB(boolean mode) {

        if (getConnectState() != ConnectState.CONNECTED) {
            return new UHFReaderResult<Boolean>(CODE_READER_NOT_CONNECTED, UHFReaderResult.ResultMessage.READER_NOT_CONNECTED, false);
        }
        return reader.setGB(mode);
    }
}
